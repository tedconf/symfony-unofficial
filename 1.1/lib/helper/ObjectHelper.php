...
function objects_for_select($options, $value_method, $text_method = null, $selected = null, $html_options = array())
{
  ...
  {
    // text method exists?
    if ($text_method && !method_exists(array($option, $text_method)))
    {
...
function object_select_tag($object, $method, $options = array(), $default_value = null)
{
  ...
  $select_options = _get_options_from_objects(sfContext::getInstance()->retrieveObjects($related_class, $peer_method, $criteria), $text_method, $key_method);
  ...
  if ($objects)
  {
    ...
    foreach ($objects as $tmp_object)
    {
      if ($first)
      {
        ...
        {
          if (method_exists(array($tmp_object, $method)))
          {
            $methodToCall = $method;
            break;
          }
...
function _get_object_value($object, $method, $default_value = null, $param = null)
{
  ...
  // method exists?
  if (!method_exists(array($object, $method[0])))
  {
