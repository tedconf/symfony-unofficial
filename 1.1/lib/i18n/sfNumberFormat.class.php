...
class sfNumberFormat
{
  ...
  /**
   * Formats an localized number in string format to a number (float or integere)
   *
   * @param string the decimal number in string form.
   * @param string decimal point type (optional)
   * @param string thousands separator (optional)
   * @return integer or float
   */
   public function formatNumber($string, $decimalPoint = null, $thousandsSeparator = null)
   {
     if(is_null($decimalPoint) || is_null($thousandsSeparator))
     {
       $groupSeparator = $this->formatInfo->GroupSeparator;
       $decimalSeparator = $this->formatInfo->DecimalSeparator;
     }

     $number = (float) str_replace($decimalSeparator, '.', str_replace($groupSeparator, '', $string));
