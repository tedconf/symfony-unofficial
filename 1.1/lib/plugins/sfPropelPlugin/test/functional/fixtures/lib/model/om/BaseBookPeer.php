...
abstract class BaseBookPeer {
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new BookMapBuilder();
		}
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($distinct || in_array(Criteria::DISTINCT, $criteria->getSelectModifiers())) {
			$criteria->addSelectColumn(BookPeer::COUNT_DISTINCT);
		} else {
			$criteria->addSelectColumn(BookPeer::COUNT);
		}
	...
	public static function doInsert($values, PropelPDO $con = null)
	{
		...
		if($criteria->containsKey(BookPeer::ID)) {
	...
	public static function doValidate(Book $obj, $cols = null)
	{
		...
		$res =  BasePeer::doValidate(BookPeer::DATABASE_NAME, BookPeer::TABLE_NAME, $columns);
    if ($res !== true) {
        $request = sfContext::getInstance()->getRequest();
        foreach ($res as $failed) {
            $col = BookPeer::translateFieldname($failed->getColumn(), BasePeer::TYPE_COLNAME, BasePeer::TYPE_PHPNAME);
