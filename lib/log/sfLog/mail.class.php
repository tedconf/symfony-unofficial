<?php
/**
 * $Header: /repository/pear/sfLog/sfLog/mail.php,v 1.21 2004/01/19 08:02:40 jon Exp $
 *
 * @version $Revision$
 * @package sfLog
 */
...
/**
 * The sfLog_mail class is a concrete implementation of the sfLog:: abstract class
...
class sfLog_mail extends sfLog
{
     ...
     * @access private
     */
    var $_subject = '[sfLog_mail] sfLog message';
    ...
    function close()
    {
        if ($this->_opened) {
            if (!empty($this->_message)) {
                ...
                $headers .= "User-Agent: sfLog_mail";

                if (mail($this->_recipient, $this->_subject, $this->_message,
                         $headers) == false) {
                    error_log("sfLog_mail: Failure executing mail()", 0);
                    return false;
                }
    ...
    function log($message, $priority = null)
    {
        ...
        $entry = sprintf("%s %s [%s] %s\n", strftime('%b %d %H:%M:%S'),
                         $this->_ident, sfLog::priorityToString($priority),
                         $message);
