 ...
 * @version    SVN: $Id: TagHelper.php 1860 2006-08-29 15:20:32Z fabien $
 */
class TagHelper extends sfHelper
{
 ...
 * @return string
 */
static function tag($name, $options = array(), $open = false)
{
  if (!$name)
  {
    return '';
...
static function content_tag($name, $content = '', $options = array())
{
  if (!$name)
  {
    return '';
...
static function _tag_options($options = array())
{
...
static function _parse_attributes($string)
{
  return is_array($string) ? $string : sfToolkit::stringToArray($string);
}

static function _get_option(&$options, $name, $default = null)
{
