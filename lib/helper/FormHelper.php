...
/**
 * Returns a <select> tag populated with all the cultures (languages and regions) in the world (or almost).
 *
 * The select_culture_tag builds off the traditional select_tag function, and is conveniently populated with
 * all the cultures in the world (sorted alphabetically). Each option in the list has the
 * culture code for its value and the native name as its display title. The data is
 * retrieved via the sfCultureInfo class, which stores a wide variety of i18n and i10n settings for various 
 * countries and cultures throughout the world. Here's an example of an <option> tag generated by the select_culture_tag:
 *
 * <samp>
 ...
 * <b>Examples:</b>
 * <code>
 *  echo select_culture_tag('language', 'de_DE');
 * </code>
 *
 * @param  string field name 
 * @param  string selected field value (culture)
 * @param  array  additional HTML compliant <select> tag parameters
 * @return string <select> tag populated with all the cultures in the world.
 * @see select_tag, options_for_select, sfCultureInfo
 */
function select_culture_tag($name, $selected = null, $options = array())
{
  $cultures = sfCultureInfo::getCultures(sfCultureInfo::SPECIFIC);
  $c = new sfCultureInfo(sfContext::getInstance()->getUser()->getCulture());

  $culture_option = _get_option($options, 'cultures');

  $namedCultures = array();
  foreach ($cultures as $culture)
  {
    if ($culture_option && !in_array($culture, $culture_option)) continue;

    $namedCultures[$culture] = $c->getNativeName($culture);
  }

  asort($namedCultures);

  $option_tags = options_for_select($namedCultures, $selected, $options);
