 ...
 * @version    SVN: $Id: EscapingHelper.php 1752 2006-08-23 07:08:02Z fabien $
 */
class EscapingHelper extends sfHelper
{
  const ESC_ENTITIES = 'esc_entities';
  const ESC_RAW = 'esc_raw';
  const ESC_JS = 'esc_js';
  const ESC_JS_NO_ENTITIES = 'esc_js_no_entities';
 ...
 */
static function esc_entities($value)
{
  // Numbers and boolean values get turned into strings which can cause problems
  // with type comparisons (e.g. === or is_int() etc).
  return is_string($value) ? htmlentities($value, self::ENT_QUOTES, sfConfig::get('sf_charset')) : $value;
}
 ...
 * @return string the escaped value
 */
static function esc_js_no_entities($value)
{
