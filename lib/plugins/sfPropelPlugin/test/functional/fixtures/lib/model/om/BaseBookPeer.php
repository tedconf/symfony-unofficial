...
abstract class BaseBookPeer {
    ...
    foreach (sfMixer::getCallables('BaseBookPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseBookPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
		...
		$criteria->remove(BookPeer::ID); // remove pkey col since this table uses auto-increment
    ...
    foreach (sfMixer::getCallables('BaseBookPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseBookPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseBookPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseBookPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseBookPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseBookPeer', $values, $con, $ret);
    }
...
//
// NOTE: This static code cannot call methods on the BookPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the BookPeer class:
//
// Propel::getDatabaseMap(BookPeer::DATABASE_NAME)->addTableBuilder(BookPeer::TABLE_NAME, BookPeer::getMapBuilder());
//
